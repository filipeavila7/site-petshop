...
def adicionar_ao_carrinho(produto_id):
    # Busca o produto
    produto = produto_model.Produto.query.get(produto_id)
    if not produto:
        return jsonify({"success": False, "mensagem": "Produto não encontrado."}), 404

    # Verifica se o produto já está no carrinho do usuário
    existente = carrinho_model.Carrinho.query.filter_by(usuario_id=current_user.id, produto_id=produto.id).first()
    if existente:
        return jsonify({"success": False, "mensagem": "Produto já está no carrinho."}), 400

    # Cria item no carrinho
    item = carrinho_model.Carrinho(usuario_id=current_user.id, produto_id=produto.id)
    db.session.add(item)
    try:
        db.session.commit()
    except Exception as e:
        ...
        return jsonify({"success": False, "mensagem": "Erro ao adicionar ao carrinho."}), 500
...
@login_required
def listar_carrinho():
    itens = carrinho_model.Carrinho.query.filter_by(usuario_id=current_user.id).all()
    resultado = []
    for it in itens:
        prod = produto_model.Produto.query.get(it.produto_id)
        resultado.append({
            "item_id": it.id,
            "produto_id": it.produto_id,
            "nome_produto": getattr(prod, "nome_produto", None),
            "valor": getattr(prod, "valor", None),
            "descricao": getattr(prod, "descricao", None),
            "imagem": getattr(prod, "imagem", None)
        })
    return jsonify(resultado)


# Rota para remover um item do carrinho (recebe o id do item no carrinho)
@app.route("/carrinho/remover/<int:item_id>", methods=["POST"])
@login_required
def remover_do_carrinho(item_id):
    item = carrinho_model.Carrinho.query.get(item_id)
    if not item:
    ...
    if item.usuario_id != current_user.id:
        return jsonify({"success": False, "mensagem": "Ação não autorizada."}), 403
    ...
    try:
        ...
        return jsonify({"success": False, "mensagem": "Erro ao remover do carrinho."}), 500

    return jsonify({"success": True, "mensagem": "Item removido do carrinho com sucesso."})


# Rota para verificar se um produto específico está no carrinho do usuário
@app.route("/carrinho/verificar/<int:produto_id>", methods=["GET"])
@login_required
def verificar_carrinho(produto_id):
    item = carrinho_model.Carrinho.query.filter_by(usuario_id=current_user.id, produto_id=produto_id).first()
    if item:
        return jsonify({"no_carrinho": True, "item_id": item.id})
    else:
        return jsonify({"no_carrinho": False})


# Rota para retornar a quantidade total de produtos no carrinho do usuário logado
@app.route("/carrinho/total", methods=["GET"])
@login_required
def total_carrinho():
    # Conta quantos registros existem no carrinho para o usuário
    total_itens = carrinho_model.Carrinho.query.filter_by(usuario_id=current_user.id).count()

    # Calcula valor total (opcional) somando o valor dos produtos no carrinho
    total_valor = (
        db.session.query(func.coalesce(func.sum(produto_model.Produto.valor), 0.0))
        .join(carrinho_model.Carrinho, produto_model.Produto.id == carrinho_model.Carrinho.produto_id)
        .filter(carrinho_model.Carrinho.usuario_id == current_user.id)
        .scalar()
    )

    return jsonify({
        "total_itens": int(total_itens),
        "total_valor": float(total_valor) if total_valor is not None else 0.0
    })
...
def criar_post():
    if request.method == "POST":
        ...
        raca = request.form.get("raca", "").strip() or "Não determinada"
        imagem = request.files.get("imagem")
        if not nome_post:
            return "Nome do post é obrigatório", 400
        if not idade:
            return "Idade é obrigatória", 400
        if not peso:
            return "Peso é obrigatório", 400
        ...
        novo_post = post_models.Post(nome_post=nome_post, imagem=imagem_path, idade=int(idade), peso=float(peso), raca=raca)
        db.session.add(novo_post)
        db.session.commit()
...
def editar_post(post_id):
    ...
    if request.method == "POST":
        ...
        post.idade = int(request.form["idade"])
        post.peso = float(request.form["peso"])
        post.raca = request.form.get("raca", "").strip() or "Não determinada"
        imagem = request.files.get("imagem")
...
def criar_produto():
    if request.method == "POST":
        ...
        if not valor:
            return "Valor é obrigatório", 400
        if not descricao:
            return "Descrição é obrigatória", 400
        if not imagem or imagem.filename == "":
            return "Imagem é obrigatória", 400
        ...
        novo_produto = produto_model.Produto(nome_produto=nome_produto, valor=float(valor), descricao=descricao, imagem=imagem_path)
...
def editar_produto(produto_id):
    produto = produto_model.Produto.query.get_or_404(produto_id)
    if request.method == "POST":
        ...
        if imagem and imagem.filename != "":
            ...
            filepath = os.path.join(upload_folder, filename)
            imagem.save(filepath)
            produto.imagem = f"img/{filename}"
        db.session.commit()
        return redirect(url_for("admin_produtos"))
    return render_template("editar_produto.html", produto=produto)


@app.route("/admin/produto/<int:produto_id>/deletar", methods=["POST"])
@login_required
@admin_required
...
@login_required
def carrinho_view():
    # Buscar itens do carrinho via a rota JSON existente
    itens = carrinho_model.Carrinho.query.filter_by(usuario_id=current_user.id).all()
    carrinho_itens = []
    for it in itens:
        prod = produto_model.Produto.query.get(it.produto_id)
