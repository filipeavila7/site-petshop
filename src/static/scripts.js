...
document.addEventListener('DOMContentLoaded', async function () {
  ...
  // Carregar produtos dinâmicos
  await carregarProdutosDinamicos();
...
async function carregarProdutosDinamicos() {
  try {
    ...
    const produtoContainer = document.querySelector(".prod-container");
    if (!produtoContainer) return;

    // Limpa os produtos estáticos
    produtoContainer.innerHTML = "";

    // Adiciona produtos dinâmicos
    produtos.forEach(produto => {
      ...
      card.innerHTML = `
        <div class="card-prod-content">
          <img src="${produto.imagem ? 'static/' + produto.imagem : 'static/img/default.jpg'}" alt="${produto.nome_produto}" onclick="window.location.href='/produto/${produto.id}'" style="cursor: pointer;">
          <h3 onclick="window.location.href='/produto/${produto.id}'" style="cursor: pointer;">${produto.nome_produto}</h3>
...
sideMenu.innerHTML = `
  <div class="side-menu-content">
    ...
    <ul>
      ...
      <li><a href="/carrinho/view">Carrinho</a></li>
    </ul>
  </div>
`;
...
window.addEventListener('click', function (event) {
  ...
  const clickedMenuButton = event.target === menuToggle;
